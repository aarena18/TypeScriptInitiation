import { ContractStatus, ClientType, AccidentType, PaymentMethod, OptionType, BeneficiaryRelation, ReimbursementMethod, ReimbursementStatus, AnomalyType, AnomalySeverity, } from "../types/index.js";
...
import { InsuranceCompany, ReinsuranceContract, ReinsuranceCoverage, } from "../models/reinsurance.js";
...
export function runPartie56Tests() {
    ...
    const alicePayment1 = new Payment(4001, aliceContract1.id, 500, PaymentMethod.CARD, new Date("2025-09-01"));
    const alicePayment2 = new Payment(4002, aliceContract1.id, 500, PaymentMethod.BANK_TRANSFER, new Date("2025-10-01"));
    const bobPayment1 = new Payment(4003, bobContract.id, 300, PaymentMethod.CASH, new Date("2025-09-15"));
    ...
    console.log(`  - Period: ${audit.startDate
        .toISOString()
    ...
    const swissRe = new InsuranceCompany(1, "Swiss Re Ltd", "Switzerland", "AA-", "contact@swissre.com");
    const munichRe = new InsuranceCompany(2, "Munich Re Group", "Germany", "AA", "info@munichre.com");
    const lloydsSyndicates = new InsuranceCompany(3, "Lloyd's of London Syndicates", "United Kingdom", "A+", "underwriting@lloyds.com");
    ...
    const swissReContract = new ReinsuranceContract(1, swissRe, new Date("2025-01-01"), new Date("2025-12-31"), 40, // 40% du risque
    50000 // Maximum 50k€ par sinistre
    );
    const munichReContract = new ReinsuranceContract(2, munichRe, new Date("2025-01-01"), new Date("2025-12-31"), 35, // 35% du risque
    40000 // Maximum 40k€ par sinistre
    );
    const lloydsContract = new ReinsuranceContract(3, lloydsSyndicates, new Date("2025-06-01"), new Date("2026-05-31"), 20, // 20% du risque
    ...
    const bySeverity = {
        critical: allAnomalies.filter((a) => a.severity === AnomalySeverity.CRITICAL).length,
